...
def main():
    ...
    if st.session_state.mode == "timer_mode":
        ...
        # Timer logic - only run if timer is active for this question
        if st.session_state.timer_active:
            ...
            # Run the timer only if it's active
            remaining_time = end_time - time.time()
            while remaining_time > 0:
                # Calculate remaining time and ratio
                ...
                # Check if still in the same question
                if st.session_state.current_question != current_idx:
                    break
                
                # Update remaining time
                remaining_time = end_time - time.time()
            
            # If timer ended naturally (not because of navigation)
            if remaining_time <= 0 and st.session_state.current_question == current_idx:
                # Save current answer if selected, otherwise mark as "No answer"
                st.session_state.timer_answers[current_idx] = selected_option if selected_option else "No answer"
                st.session_state.timer_active = False
                next_question()
                st.rerun()

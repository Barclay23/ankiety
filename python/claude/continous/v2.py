...
# New variables for consistent timer behavior
if 'timer_start_time' not in st.session_state:
    st.session_state.timer_start_time = time.time()
if 'question_change_time' not in st.session_state:
    st.session_state.question_change_time = time.time()
...
def timer_handler(current_idx, time_limit, selected_option):
    # Check if we need to initialize the timer for this question
    current_time = time.time()
    if st.session_state.question_change_time == current_time or st.session_state.current_question != current_idx:
        st.session_state.timer_start_time = current_time
        st.session_state.question_change_time = current_time
    ...
    if remaining_time <= 0:
        ...
        next_question()
        return True  # Indicates a rerun is needed
...
def main():
    ...
    if st.session_state.mode == "timer_mode":
        ...
        # Handle timer logic separately without blocking UI
        if st.session_state.timer_active:
            ...
            # Check if time's up - if so, move to next question
            if remaining_time <= 0:
                st.session_state.timer_answers[current_idx] = selected_option if selected_option else "No answer"
                st.session_state.timer_active = False
                next_question()
        ...
        # Add a small delay with empty container to allow UI updates without blocking
        with st.empty():
            time.sleep(0.1)

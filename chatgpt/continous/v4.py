...
def inject_css():
    st.markdown(
        ...
        <style>
            ...
            .stMarkdown, .stText, .stCaption, .stSubheader, .stTitle, .stHeader, .stDataFrame {
                color: white !important;
            }

            /* Labels like radio buttons, checkboxes */
            ...
            /* Fix form label hover/focus */
            .css-1cpxqw2, .css-qrbaxs, .css-10trblm, .css-15zrgzn {
                color: white !important;
            }
        </style>
        """,
...
def show_results():
    ...
    for i, (ans, secs) in enumerate(zip(st.session_state.answers["timer"], st.session_state.times["timer"])):
        st.write(f"**Q{i+1}:** {ans} _(Time: {secs:.2f}s)_")
